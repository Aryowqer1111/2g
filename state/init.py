...
def init_game():
    """Инициализация ядра игры и движков"""
    if "state" not in st.session_state:
        st.session_state.state = get_clean_state()
        st.session_state.loop = GameLoop()
    
    # Инициализация движков (только если ещё не созданы)
    ...
    # Синхронизация данных из схемы в плоский session_state
    for k, v in st.session_state.state.items():
        if k not in ["career", "faction_engine", "committee_engine", 
                     "skills_engine", "loop", "cc_members", "player_profile"]:
            st.session_state[k] = v


def init_cc_data():
    """Генерация ЦК, регионов, навыков и фракций"""
    if "cc_members" not in st.session_state or st.session_state.cc_members is None:
        ...
        if "regions" not in st.session_state:
            regs = {
                rid: {"active": True, "plan_fulfillment": 85.0, "loyalty": 70.0} 
                for rid in REGION_META
            }
            st.session_state.state["regions"] = regs
            st.session_state.regions = regs
        
        # Генерация навыков и лидеров фракций
        if hasattr(st.session_state, "skills_engine"):
