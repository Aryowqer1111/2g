...
def render_career_tab():
    ...
    # 🔹 ИСПРАВЛЕНО: Ключи должны быть БЕЗ пробелов!
    # Если в schema.py или init.py у вас остались пробелы ("career "),
    # этот код их не найдет. Убедитесь, что ключи чистые.
    career = st.session_state.get("career")
    profile = st.session_state.get("player_profile")

    # Авто-восстановление карьеры, если она слетела
    if career is None:
    ...
    # Безопасный доступ к REGION_META
    if career.region_assignment and career.region_assignment in REGION_META:
        region_name = REGION_META[career.region_assignment].get("name", "Без имени")

    # 🔹 ИСПРАВЛЕНО: Убраны пробелы в ключах профиля (name, age, faction, id)
    # Важно: character_creation.py должен сохранять данные именно так (без пробелов)!
    ...
    with col2:
        ...
        progress_val = min(career.score / max(threshold, 1), 1.0)
    ...
    # 🔹 ИСПРАВЛЕНО: Вернул "мес." вместо "нед."
